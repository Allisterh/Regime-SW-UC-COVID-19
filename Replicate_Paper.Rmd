...
```{r}
GridSearch_fctn(model = modelSpec, data = dataVec, trendIni = trendIni, nRandomGrid = 3e04, stepsGrid = 6, storeOutput = F)
```
...
```{r}
Figures_fctn(
  model = modelSpec, modelOutput = modelOutputList, dataTib = dataTib,
  dataExten = dataTibExten, latexOutput = T
)
```
