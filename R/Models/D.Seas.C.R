...
ParConstrain_fctn <- function(par) {
  ...
  phi_1 <- par[4] / (1 + abs(par[4]))
  phi_2 <- (1 - abs(phi_1)) * par[5] / (1 + abs(par[5])) + abs(phi_1) - phi_1^2
  constrPar[4] <- 2 * phi_1
  constrPar[5] <- -(phi_1^2 + phi_2)
...
ParList_fctn <- function(par, constrainPar) {
  ...
  eta <- par[2] # Sd of innovations to the cycle component in regime 0
  nu_1 <- par[3] # Additional drift for down-turning regime 1
  phi_1 <- par[4] # AR covariate of the cycle
  phi_2 <- par[5] # AR covariate of the cycle
...
#' heterogeneity
#' @param nRandomGrid number of random parameter vectors
#' @return tibble with the random parameter vectors

ThetaRand_fctn <- function(nRandomGrid) {
  thetaRand <- tibble(
    xi = log(runif(nRandomGrid, 0.01, 0.25)),
    eta = log(runif(nRandomGrid, 0.01, 0.3)),
    nu_1 = log(runif(nRandomGrid, 0.001, 0.3)),
    phi_1 = runif(nRandomGrid, -7, 7),
    phi_2 = runif(nRandomGrid, -7, 7),
    q = runif(nRandomGrid, -5, 5),
    p = runif(nRandomGrid, -5, 5)
  )
  return(thetaRand)
}
...
outputParam_fctn <- function(paramList) {
  return(c(
    "xi" = paramList$xi$xi_0, "eta" = paramList$eta$eta_0, "nu_1" = paramList$nu_1, "phi_1" = paramList$phi[1],
    "phi_2" = paramList$phi[2], "q" = paramList$Probs$q, "p" = paramList$Probs$p
  ))
}
...
simData_fctn <- function(nPeriods) {
  ...
  trend <- simTrend_fctn(nPeriods = nPeriods, regimeVec = regime)
...
paramIni_fctn <- function() {
  return(c(
    "xi" = 1e5, "eta" = 1e5, "nu_1" = -.5,
    "phi_1" = 0, "phi_2" = 0, "q" = .95, "p" = .95
  ))
}
