...
AdditionalParam_fctn <- function(data, stateVec, regimeVec, paramList) {
  ...
  # eta
  eta <- SdEta_fctn(stateVec = stateVec, regimeHeteroscVec = regimeVec, paramList = paramList, cyclePos = 3)
  paramList$eta$eta_0 <- eta[1]
  paramList$eta$eta_1 <- eta[2]
  # phi
...
outputParam_fctn <- function(paramList) {
  return(c(
    ...
    "nu_1" = paramList$nu_1, "phi_1" = paramList$phi[1], "phi_2" = paramList$phi[2], "q" = paramList$Probs$q, "p" = paramList$Probs$p
  ))
}
...
simData_fctn <- function(nPeriods) {
  ...
  cycle <- simCycle_fctn(nPeriods, regimeHeteroscVec = regime)
  y <- trend + seas + cycle
  return(y)
}
...
paramVec_fctn <- function() {
  return(c(
    "xi" = 1e4, "omega" = 1e4, "eta_0" = 1e4, "eta_1" = 1e4, "nu_1" = -.5,
