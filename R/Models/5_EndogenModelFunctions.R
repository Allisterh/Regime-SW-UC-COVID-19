...
InvParConstrain_fctn <- function(par) {
  ...
  correction[c(1:3, 5)] <- diag(numDeriv::jacobian(exp, par[c(1:3, 5)]))
  # Constrains variance to be between 0 and 1
  correction[4] <- diag(numDeriv::jacobian(function(x) 1 / (1 + exp(x)), par[4]))
  # Defines regimes by nu_1 < 0
  correction[5] <- -exp(par[5])
...
ParList_fctn <- function(par, constrainPar) {
  ...
  varrho <- par[4] # Variance between trend and regime
  nu_1 <- par[5] # Additional drift for down-turning regime 1
  beta_0 <- par[6] # Probability of staying in regime 0
  beta_1 <- par[7] # Probability of staying in regime 1
