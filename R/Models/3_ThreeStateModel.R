...
Likelihood_fctn <- function(par, data, Ini) {
  ...
  P <- matrix(NA, 3, 3)
  P[, 1] <- c(1 - par[5] - par[6], par[5], par[6]) # P(St =  | St-1 = 0)
  P[, 2] <- c(1 - par[7] - par[8], par[7], par[8]) # P(St =  | St-1 = 1)
  P[, 3] <- c(1 - par[9] - par[10], par[9], par[10]) # P(St =  | St-1 = 1)
  ...
  for (i in 6:periods) {
    ...
    # Regime 2 (no drift)
    a_t_clt_02 <- Tt %*% (a_t_ct_0 + matrix(c(0, -a_t_ct_0[2, 1], rep(0, Dimens - 2)), nc = 1))
    a_t_clt_12 <- Tt %*% (a_t_ct_1 + matrix(c(0, -a_t_ct_1[2, 1], rep(0, Dimens - 2)), nc = 1))
    a_t_clt_22 <- Tt %*% (a_t_ct_2 + matrix(c(0, -a_t_ct_2[2, 1], rep(0, Dimens - 2)), nc = 1))
...
Filter_fctn <- function(par, data, Ini) {
  ...
  for (i in 6:periods) {
    ...
    # Regime 2 (no drift)
    a_t_clt_02 <- Tt %*% (a_t_ct_0 + matrix(c(0, 0, -a_t_ct_0[3, 1], rep(0, Dimens - 3)), nc = 1))
    a_t_clt_12 <- Tt %*% (a_t_ct_1 + matrix(c(0, 0, -a_t_ct_1[3, 1], rep(0, Dimens - 3)), nc = 1))
    a_t_clt_22 <- Tt %*% (a_t_ct_2 + matrix(c(0, 0, -a_t_ct_2[3, 1], rep(0, Dimens - 3)), nc = 1))
    P_t_clt_02 <- Tt %*% P_t_ct_0 %*% TranspTt + expandQ
    P_t_clt_12 <- Tt %*% P_t_ct_1 %*% TranspTt + expandQ
    P_t_clt_22 <- Tt %*% P_t_ct_2 %*% TranspTt + expandQ
  }
...
Smoother_fctn <- function(par, filterOutput, ciInterval) {
  ...
  P_cT_array[, (3 * Dimens + 1):(4 * Dimens), periods] <- sum(Pr_cT_0 * filterOutput[["P_ct"]][, 1:Dimens, periods],
                                                              ...
                                                              Pr_cT_3 * filterOutput[["P_ct"]][, (2 * Dimens + 1):(3 * Dimens), periods])
