...
AdditionalParam_fctn <- function(data, stateVec, regimeVec, paramList) {
  ...
  # xi
  xiVec <- SdXiSwitch_fctn(stateVec = stateVec, regimeDriftVec = regimeVec, regimeHeteroscVec = regimeVec, paramList = paramList)
...
simData_fctn <- function(nPeriods) {
  ...
  trend <- simTrend_fctn(nPeriods = nPeriods, regimeVec = regime, heteroscInd = TRUE)
...
paramIni_fctn <- function(){
  return(c("xi_0" = 1e4, "xi_1" = 1e4, "omega" = 1e4, "epsilon" = 1e4, 
           "nu_1" = -.5, "q" = .95, "p" = .95))
}